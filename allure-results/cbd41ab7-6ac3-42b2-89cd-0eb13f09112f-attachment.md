...
Call log:
  - waiting for locator('//div[@class=\'productinfo\' and contains(., \'Blue Top\')]').first()

    at ProductsPage.addProductToCartByName (/app/pages/ProductsPage.js:96:39)
    at /app/tests/placeOrderRegisterWhileCheckout.spec.js:40:28
```
...
```ts
   ...
   28 |         this.productItemByName = (name) => `//div[@class='productinfo' and contains(., '${name}')]`; // XPath to find product container by name
...
>  96 |         await productContainerLocator.hover(); // Hover to ensure button is visible if needed
  ...
  104 |         return (await priceElement.textContent()).trim();
  105 |     }
  106 |
